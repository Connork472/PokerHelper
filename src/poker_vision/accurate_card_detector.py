#!/usr/bin/env python3
"""
Accurate Poker Card Detector with Poker Simulator Integration
Fixes preprocessing, adds temporal smoothing, and integrates win probability
"""
...
# Import poker simulator components
import sys
import os
...
class AccurateCardDetector:
    def __init__(self):
        ...
        # Poker simulator integration
        self.poker_thread = None
    ...
    def start_poker_simulator(self, hand_cards, board_cards):
        """Start poker simulator in background thread"""
        if self.simulator_running:
            return
            
        self.simulator_running = True
        self.poker_thread = threading.Thread(
            target=self.run_poker_simulation,
            args=(hand_cards, board_cards),
            daemon=True
        )
        self.poker_thread.start()
    
    def run_poker_simulation(self, hand_cards, board_cards):
        """Run poker simulation in background"""
        try:
            # Convert cards to treys format
            hand_str = hand_cards
            board_str = board_cards
            
            # Validate cards
            try:
                my_cards = to_treys(hand_str)
                board_cards_treys = to_treys(board_str)
            except Exception as e:
                if self.debug_mode:
            ...
            # Run simulation
            players = 6  # Default
            trials = 10000  # Faster for real-time
            
            win_p, tie_p, equity = simulate(players, my_cards, board_cards_treys, trials)
            kelly = kelly_even_money(equity)
            
            self.current_equity = equity
            self.current_kelly = kelly
            
            if self.debug_mode:
                print(f"Equity: {equity*100:.1f}%, Kelly: {kelly*100:.1f}%")
    ...
    def run_accurate_detection(self):
        ...
        print("Accurate Poker Card Detector with Win Probability")
        print("=" * 60)
        print("🎯 100% Accurate Detection + Poker Simulator Integration")
        print()
        print("CONTROLS:")
        ...
        print("STEP 4: Watch win probability update as cards are revealed")
        print()
        ...
        while True:
            ...
            if self.mode == "select_hand":
                ...
                if detection_active:
                    status = "DETECTING CARDS + CALCULATING WIN PROBABILITY"
                    color = (255, 255, 0)
                else:
            ...
            if detection_active:
                ...
                # Show win probability if available
                if self.current_equity is not None:
                    equity_str = f"Equity: {self.current_equity*100:.1f}%"
                    kelly_str = f"Kelly: {self.current_kelly*100:.1f}%"
                    cv2.putText(display_frame, equity_str, (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
                    cv2.putText(display_frame, kelly_str, (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
                
                # Start poker simulation if we have cards
                if hand_cards and not self.simulator_running:
                    self.start_poker_simulator(hand_cards, board_cards)
            ...
            if key == ord('q'):
                ...
                print("Starting continuous detection with win probability calculation...")
            elif key == ord('b'):
